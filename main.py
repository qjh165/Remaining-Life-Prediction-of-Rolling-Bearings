        ...
        if results:
            ...
            if summary_data:
                ...
                for col in df_summary.columns:
                    if 'hi_r2' in col or 'phm_score' in col:
    ...
    try:
        ...
        if all_results:
            ...
            if not df_summary.empty:
                ...
                phm_cols = [col for col in df_summary.columns if 'phm_score' in col]
                for col in phm_cols:
                    if col in df_summary.columns:
                        phm_mean = df_summary[col].mean()
                        phm_std = df_summary[col].std()
                        print(f"{col}: 平均={phm_mean:.4f}, 标准差={phm_std:.4f}")
                
                print(f"\n详细结果:")
                for idx, row in df_summary.iterrows():
                    bearing_name = row.get('bearing_name', f'轴承_{idx+1}')
                    hi_r2 = row.get('multimodal_hi_r2', 0)
                    phm = row.get('multimodal_phm_score', 0)
                    print(f"{idx+1}. {bearing_name}: HI-R²={hi_r2:.4f}, PHM={phm:.4f}")
...
if __name__ == "__main__":
    ...
    try:
        choice = input("请选择 (1-8, 默认1): ").strip()
